...
fn reg_send(lua: &mlua::Lua) -> anyhow::Result<()> {
    let span = tracing::debug_span!("req_send");
    let _enter = span.enter();

    let globals = lua.globals();

    let send_fn = lua
        .create_function(|lua, args: mlua::Table| {
        ...
        .map_err(|e| {
            tracing::error!("Failed to create GET function: {}", e);
            anyhow::anyhow!("Failed to create GET function")
        })?;
    globals.set("send", send_fn).map_err(|e| {
        tracing::error!("Failed to set GET function in globals: {}", e);
        anyhow::anyhow!("Failed to set GET function in globals")
    })?;
...
fn reg_print_response(lua: &mlua::Lua) -> anyhow::Result<()> {
    let span = tracing::debug_span!("reg_print_response");
    let _enter = span.enter();

    let globals = lua.globals();

    let print_response_fn = lua
        .create_function(|_, response: mlua::Table| {
        ...
        .map_err(|e| {
            tracing::error!("Failed to create print_response function: {}", e);
            anyhow::anyhow!("Failed to create print_response function")
        })?;
    globals
        .set("print_response", print_response_fn)
        .map_err(|e| {
            tracing::error!("Failed to set print_response function in globals: {}", e);
            anyhow::anyhow!("Failed to set print_response function in globals")
        })?;
